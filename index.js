...
app.get("/genres/:genreName", (req, res) => {
  ...
  const genre = genres.find((genre) => genre.name === genreName).name;
  if (genre) {
...
app.get(" /directors/:name", (req, res) => {
  ...
  const director = directors.find((name) => director.name === name);
  if (name) {
    res.status(200).json(name);
  } else {
    res.status(400).send("The director " + name + " was not found");
  }
});
...
app.put("/users/:username", (req, res) => {
  ...
  if (user) {
    ...
    if (updatedUser.firstName) {
      user.email = updatedUser.firstName;
      res
        .status(201)
        .send(
    ...
    if (updatedUser.lastName) {
      user.email = updatedUser.lastName;
      res
        .status(201)
        .send(
    ...
    if (updatedUser.password) {
      ...
      res
        .status(201)
        .send("The password for User " + username + " was updated");
    }
  } else {
...
app.get(" /directors/id/:name", (req, res) => {
  const { name } = req.params;
  const director = directors.find((name) => director.name === name).directorID;
  if (name) {
    res.status(200).json(name);
  } else {
    res.status(400).send("The director " + name + " was not found");
